...
void CHZGoogleDialog::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (point.y > 40 || point.x > nBorder)
	{
		...
		for (int i = 0; i < m_arrThumbs.GetCount(); i++)
		{
			...
			rcFocus = CRect(CPoint(nLeft, nTop), CSize(nSquare, nSquare));

			if (rcFocus.PtInRect(point))
			{
				nIndex = i;
				CRect rcCrop = CRect(point.x - nEdge / 2, point.y - nEdge / 2, point.x + nEdge / 2, point.y + nEdge / 2);
				...
				if (rcCrop.left > (nTop + nHeight)) rcCrop.MoveToY(nTop + nHeight - nEdge);
				m_rcCrop = rcCrop;
				break;
			}
		}
